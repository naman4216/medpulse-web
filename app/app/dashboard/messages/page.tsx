...
export default function MessagesPage() {
  ...
  const handleSendMessage = useCallback(() => {
    ...
    if (editingMessage && editingMessage.type === 'message') {
      setChatHistories(prev => ({ ...prev, [selectedConversation.id]: prev[selectedConversation.id].map(msg => msg.id === editingMessage.id ? { ...msg, text: messageInput, isEdited: true } : msg) }));
      setEditingMessage(null);
    } else { // Handle New Message
    ...
    // Simulate status progression for new messages
    if (!editingMessage) {
  ...
  const handleDeleteMessage = (messageId: number) => {
    setChatHistories(prev => ({ ...prev, [selectedConversation.id]: prev[selectedConversation.id].filter(msg => msg.id !== messageId) }));
  ...
  const handleToggleReaction = (messageId: number, emoji: string) => {
    setChatHistories(prev => ({
      ...prev, [selectedConversation.id]: prev[selectedConversation.id].map(msg => {
        if (msg.id === messageId && msg.type === 'message') {
          const newReactions = { ...(msg.reactions || {}) };
          // In a real app, you'd check if the *user* has reacted. Here we just toggle.
                  ...
                  <div key={msg.id} className={`flex gap-3 items-end group ${msg.sender === 'me' ? 'flex-row-reverse' : 'flex-row'}`}>
                    <Popover>
                      <div className={`flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ${msg.sender === 'me' ? 'flex-row-reverse' : ''}`}>
                        <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="w-7 h-7" onClick={() => (msg.sender === 'patient' && handleStartReply(msg))}><Reply className="w-4 h-4" /></Button></TooltipTrigger><TooltipContent><p>Reply</p></TooltipContent></Tooltip>
          ...
          <aside className="w-1/4 min-w-[350px] max-w-[400px] border-l border-slate-200 flex flex-col bg-white">
            <header className="p-4 border-b border-slate-200 text-center">
              ...
              <div className="flex justify-center gap-2 mt-4">
                <Button variant={selectedConversation.starred ? "default" : "outline"} size="icon" onClick={() => { }}><Star className="w-4 h-4" /></Button>
                <Button variant="outline" size="icon" onClick={() => { }}><AlertCircle className={`w-4 h-4 ${selectedConversation.priority === 'urgent' ? 'text-red-500' : ''}`} /></Button>
                <Button variant="outline" size="icon" onClick={() => { }}><Archive className="w-4 h-4" /></Button>
              </div>
            </header>
              ...
              <div className="flex-1 overflow-y-auto">
                <TabsContent value="details" className="p-4 space-y-4">
                  <div className="text-sm">
                    <span className="font-medium">Phone:</span> {selectedConversation.phone}
                  </div>
                  <div className="text-sm">
                    <span className="font-medium">Email:</span> {selectedConversation.email}
                  </div>
                  <div className="text-sm">
                    <span className="font-medium">Next Appointment:</span> {selectedConversation.appointment}
                  </div>
                </TabsContent>
                ...
                <TabsContent value="history" className="p-4 space-y-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedConversation.allergies.map(a => (
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedConversation.chronic.map(c => (
                ...
                <TabsContent value="vitals" className="p-4 space-y-2">
                  ...
                  {vitals[selectedConversation.id]?.map((v, i) => (
                    <Card key={i}>
                      <CardContent className="p-3 text-xs grid grid-cols-2 gap-1">
                        <div><span className="font-medium">BP:</span> {v.bp}</div>
                        <div><span className="font-medium">HR:</span> {v.hr} bpm</div>
                        <div><span className="font-medium">Temp:</span> {v.temp}°C</div>
                        <div><span className="font-medium">SpO2:</span> {v.spo2}%</div>
                        <div className="col-span-2 text-slate-500">{v.date}</div>
                      </CardContent>
                    </Card>
                  ))}
                </TabsContent>
                ...
                <TabsContent value="notes" className="p-4 space-y-2 h-full flex flex-col">
                  ...
                  <div className="flex-1 space-y-2 overflow-y-auto">
                    {clinicalNotes[selectedConversation.id]?.map(n => (
                      <Card key={n.id}>
                  ...
                  <div className="mt-auto pt-2">
                    <Textarea
                      ...
                      className="mb-2"
                    />
                    <Button size="sm" className="w-full" onClick={saveNewClinicalNote}>
                      Save Note
                    </Button>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="files" className="p-4 space-y-2">
                  {currentChat
                    .filter(msg => msg.type === 'message' && msg.attachment)
                    .map((msg, idx) => (
        ...
        {videoCall.isActive && (
          <div className={`fixed inset-0 bg-slate-900 z-50 flex flex-col ${videoCall.isFullscreen ? '' : 'p-4'}`}>
            <div className="flex-1 relative grid grid-cols-2 gap-2">
              {/* Patient Video */}
              <div className="relative bg-slate-800 rounded-lg overflow-hidden flex items-center justify-center">
                <Avatar className="w-32 h-32 border-4 border-white"><AvatarImage src={selectedConversation.avatar} /><AvatarFallback className="text-4xl">{selectedConversation.name.charAt(0)}</AvatarFallback></Avatar>
                <div className="absolute bottom-4 left-4 bg-black/70 px-3 py-1.5 rounded-lg flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full"></div><span className="text-white font-medium">{selectedConversation.name}</span></div>
              </div>
              {/* Doctor Video */}
              <div className="relative bg-slate-800 rounded-lg overflow-hidden flex items-center justify-center">
                {videoCall.isVideoOn ? <Avatar className="w-32 h-32 border-4 border-white"><AvatarImage src="/avatars/doctor.png" /><AvatarFallback className="text-4xl">Dr</AvatarFallback></Avatar> : <div className="text-center text-white"><VideoOff className="w-16 h-16 mx-auto mb-2" />Camera Off</div>}
                <div className="absolute bottom-4 left-4 bg-black/70 px-3 py-1.5 rounded-lg flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full"></div><span className="text-white font-medium">You</span>{!videoCall.isAudioOn && <MicOff className="w-4 h-4 text-red-500" />}</div>
                {videoCall.isRecording && <div className="absolute top-4 right-4 bg-red-600 px-3 py-1.5 rounded-lg flex items-center gap-2 animate-pulse"><div className="w-2 h-2 bg-white rounded-full"></div><span className="text-white text-sm font-medium">REC</span></div>}
              </div>
