...
export default function MessagesPage() {
  ...
  // Handle patient query param and video call trigger
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const patientId = urlParams.get('patient');
    const startVideo = urlParams.get('video') === 'true';
    
    if (patientId) {
      const patient = conversations.find(c => c.id === parseInt(patientId));
      if (patient) {
        setSelectedConversation(patient);
        if (startVideo) {
          setTimeout(() => {
            setVideoCall(prev => ({ ...prev, isActive: true, duration: 0 }));
            sendSystemMessage("📹 Video consultation started");
          }, 500);
        }
      }
    }
        ...
        <main className="flex-1 flex flex-col">
          <header className="flex items-center justify-between p-4 border-b border-slate-200 bg-white">
                <div className="flex items-center gap-4">
                  <Avatar className="w-10 h-10 cursor-pointer" onClick={() => router.push(`/dashboard/patients/${selectedConversation.id}/chart`)}>
                    <AvatarImage src={selectedConversation.avatar} />
                    <AvatarFallback>{selectedConversation.name.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div className="cursor-pointer" onClick={() => router.push(`/dashboard/patients/${selectedConversation.id}/chart`)}>
                    <h2 className="font-medium text-lg hover:underline">{selectedConversation.name}</h2>
                    <span className="text-sm text-slate-500">{selectedConversation.online ? 'Online' : 'Offline'}</span>
                  </div>
                </div>
